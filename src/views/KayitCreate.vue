            ...
            <div class="row mb-xl-8">
              <div class="col-xl-6 w-50">
                ...
                <input
                  ...
                  class="form-control"
                  @change="fileProps()"
                  :class="['arac_dosyalar[]', this.dosyaValidation]"
                />
                <span class="text-muted">
                  *
                ...
                <br />
                <span
                  v-if="this.fileProp[0]?.size"
                  class="text-right text-muted"
                >
                  <strong>Dosya boyutu:</strong>
                  {{ this.fileProp[0]?.size }} Byte
                </span>
                <div class="fv-plugins-message-container invalid-feedback">
...
export default {
  ...
  methods: {
    ...
    async hesapla() {
      const ozelliklerIds = this.seciliOzellikler.map((item) => item.id);
    ...
    getBeyinMarkasi() {
      axios
        ...
        .then((response) => {
          this.beyinMarka = response.data.results.sort((a, b) =>
            a.text.localeCompare(b.text, "tr", { sensitivity: "base" })
          );
        })
        .catch((error) => {
          console.log(error);
    ...
    addNew() {
      ...
      if (this.seciliOzellikler.length === 0) {
        ...
        paramet.append("ozellikler_json", JSON.stringify(this.ozelliklerJSON));
        paramet.append("bayi_id", this.bayi_id);
        paramet.append("master", this.master);
        paramet.append(
          "column_set_id",
          this.$store.getters.getUserData.auths.tables["kayitlar"]?.creates[0]
        );
        ...
        axios
          .post(
            this.$store.getters.getToken + "/tables/kayitlar/store",
            paramet
          )
          .then((res) => {
            console.log(res);
            if (res.data.code == 400) {
              ...
              toastr.success(
                "Yeni kayıt başarılı bir şekilde oluşturuldu.",
                "Kayıt Başarılı!"
              );
